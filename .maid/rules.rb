...
Maid.rules do
  ...
  rule 'Clear downloads folder' do
    trash(dir('~/Downloads/*'))
  end
  ...
  rule 'Move markdown files on desktop to Google Drive' do
    dir('~/Desktop/*.md').each do |path|
      move(path, '~/Google Drive/My Drive/')
    end
  end
